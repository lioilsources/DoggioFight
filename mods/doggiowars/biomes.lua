...
local function init_content_ids()
    ...
    c.stone      = minetest.get_content_id("dw_nodes:stone")
    c.dirt       = id("dw_nodes:dirt",                 c.stone)
    c.grass      = id("dw_nodes:dirt_with_grass",       c.dirt)
    c.ice        = id("dw_nodes:ice",                   c.stone)
    c.cave_ice   = id("dw_nodes:cave_ice",              c.ice)
    c.snow       = id("dw_nodes:snowblock",             c.ice)
    c.permafrost = id("dw_nodes:permafrost",            c.stone)
    c.obsidian   = id("dw_nodes:obsidian",              c.stone)
    c.lava       = id("dw_nodes:lava_source",           c.stone)
    c.sand       = id("dw_nodes:sand",                  c.dirt)
    c.desert_sand   = id("dw_nodes:desert_sand",        c.sand)
    c.desert_stone  = id("dw_nodes:desert_stone",       c.stone)
    c.desert_ss     = id("dw_nodes:desert_sandstone",   c.desert_stone)
    c.silver_sand   = id("dw_nodes:silver_sand",        c.sand)
    c.sandstone  = id("dw_nodes:sandstone",             c.stone)
    c.water      = id("dw_nodes:water_source",          c.air)
    c.coral      = id("dw_nodes:coral_skeleton",        c.stone)
    c.clay       = id("dw_nodes:clay",                  c.dirt)
    c.gravel     = id("dw_nodes:gravel",                c.stone)
    c.rainforest = id("dw_nodes:dirt_with_rainforest_litter", c.grass)
    c.dry_grass  = id("dw_nodes:dirt_with_dry_grass",   c.grass)
    c.dry_dirt   = id("dw_nodes:dry_dirt",              c.dirt)
